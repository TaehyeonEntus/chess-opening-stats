        ...
        <div className="space-y-3 text-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className={cn("h-2 w-2 rounded-full shadow-[0_0_8px_rgba(16,185,129,0.5)]", getWinRateColor(winRate, "bg"))} />
              <span className="font-medium">{t("wins")}</span>
            </div>
            <span className={cn("tabular-nums font-semibold", getWinRateColor(winRate))}>
  ...
  return (
    <section className="flex h-full min-w-0 flex-col gap-6 p-6 overflow-hidden">
      ...
      <div className="flex gap-4 min-w-0 overflow-hidden items-center p-3 rounded-xl bg-muted/30 border border-muted-foreground/10">
        <div className="pointer-events-none aspect-square w-16 flex-none overflow-hidden rounded-lg border shadow-sm sm:w-24" aria-hidden="true">
          <Chessboard
            key={topOpening.epd}
            options={{
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-center gap-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="secondary" className="font-mono text-[10px] h-4 px-1 leading-none shrink-0">
              {topOpening.eco}
            </Badge>
            <div className="flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground shrink-0">
              ...
              <span className={cn("font-bold tabular-nums", getWinRateColor(topOpening.winRate))}>{topOpening.winRate}%</span>
            </div>
          </div>
        </div>
      </div>
            ...
            >
              <div className="flex min-w-0 flex-1 flex-col gap-0.5 pr-3">
                ...
                <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-medium">
                  <span className="font-mono shrink-0">{op.eco}</span>
                  <span className="tabular-nums shrink-0">{op.totalGames.toLocaleString()} {t("games")}</span>
                </div>
              </div>
              <div className="flex-none text-right">
                <div className={cn("font-bold tabular-nums", getWinRateColor(op.winRate))}>{op.winRate}%</div>
              </div>
            </div>
          ))}
